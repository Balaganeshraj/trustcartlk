  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border-2 border-orange-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-semibold text-black">Pricing Configuration</h3>
          <p className="text-gray-700 mt-1">Configure your global pricing parameters and strategies</p>
        </div>
      ...
      {productCount > 0 && onApplyToAllProducts && (
        <div className="mb-8 p-6 bg-gradient-to-r from-orange-50 to-yellow-50 border-2 border-orange-200 rounded-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h4 className="text-lg font-semibold text-black">Recalculate All Prices</h4>
                <p className="text-gray-700">Apply current pricing configuration to all {productCount} products</p>
              </div>
            </div>
            <button
              onClick={onApplyToAllProducts}
              className="flex items-center space-x-2 bg-gradient-to-r from-orange-500 to-yellow-500 text-black px-6 py-3 rounded-lg hover:from-orange-600 hover:to-yellow-600 transition-all duration-200 shadow-lg hover:shadow-xl font-medium"
            >
              <RefreshCw className="w-5 h-5" />
              <span className="font-medium">Apply to All Products</span>
            </button>
          </div>
          <div className="mt-4 text-sm text-orange-800 bg-orange-100 p-3 rounded-lg">
      ...
      {/* Quick Presets */}
      <div className="mb-8">
        <h4 className="text-lg font-medium text-black mb-4">Quick Presets</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {presetConfigs.map((preset, index) => (
            <div
              key={index}
              className="border-2 border-orange-200 rounded-lg p-4 hover:border-orange-400 cursor-pointer transition-colors bg-orange-50 hover:bg-orange-100"
              onClick={() => setConfig(preset.config)}
            >
              <h5 className="font-medium text-black mb-1">{preset.name}</h5>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <label className="text-sm font-medium text-black">Profit Margin (%)</label>
            </div>
            <input
              type="number"
              ...
              step="1"
            />
            <p className="text-xs text-orange-700 mt-2">Target profit percentage on cost price</p>
          </div>
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-4 rounded-lg border border-orange-200">
            ...
            <p className="text-xs text-orange-700 mt-2">Display currency for all prices</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-orange-50 to-red-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <label className="text-sm font-medium text-black">Ad Cost per Product ({config.currency})</label>
            </div>
            <input
              type="number"
              value={config.adCost}
              ...
              step="1"
            />
            <p className="text-xs text-orange-700 mt-2">Average advertising cost per product</p>
          </div>

          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center space-x-3 mb-3">
              <Truck className="w-5 h-5 text-orange-600" />
              <label className="text-sm font-medium text-black">Delivery Cost ({config.currency})</label>
            </div>
            <input
              type="number"
        ...
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-red-50 to-orange-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center space-x-3 mb-3">
              <Calculator className="w-5 h-5 text-orange-600" />
              <label className="text-sm font-medium text-black">Tax Rate (%)</label>
            </div>
            <input
              type="number"
              ...
              step="0.1"
            />
            <p className="text-xs text-orange-700 mt-2">Applicable tax rate on sales</p>
          </div>

          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center space-x-3 mb-3">
              <CreditCard className="w-5 h-5 text-orange-600" />
              <label className="text-sm font-medium text-black">Payment Gateway Fee (%)</label>
            </div>
            <input
              type="number"
              ...
              step="0.1"
            />
            <p className="text-xs text-orange-700 mt-2">Payment processing fee percentage</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 p-6 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg border-2 border-orange-200">
        <h4 className="text-lg font-medium text-black mb-3">Pricing Formula</h4>
